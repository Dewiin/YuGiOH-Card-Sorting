...
{
  if((i < j && first_ != nullptr) || (first_->getNext() != nullptr) || (i != j))
  {
    Node<T> *i_ptr = getPointerTo(i);
    Node<T> *j_ptr = getPointerTo(j);
    Node<T> *temp = nullptr;

    if((i_ptr != nullptr) && (j_ptr != nullptr))
    {
      //EDGE CASES FOR HEAD NODE
      if(i_ptr == first_){first_ = j_ptr;}
      else if(j_ptr == first_){first_ = i_ptr;}
      //EDGE CASES FOR TAIL NODE
      if(i_ptr == last_){last_ = j_ptr;}
      else if(j_ptr == last_){last_ = i_ptr;}

      //Swapping next nodes
      temp = i_ptr->getNext();
      i_ptr->setNext(j_ptr->getNext());
      j_ptr->setNext(temp);

      //reconnecting position of i and j nodes 
      if(i_ptr->getNext() != nullptr){
        i_ptr->getNext()->setPrevious(i_ptr);
      }
      if(j_ptr->getNext() != nullptr){
        j_ptr->getNext()->setPrevious(j_ptr);
      }

      //Swapping previous nodes
      temp = i_ptr->getPrevious();
      i_ptr->setPrevious(j_ptr->getPrevious());
      j_ptr->setPrevious(temp);

      //reconnecting position of i and and j nodes
      if(i_ptr->getPrevious() != nullptr){
        i_ptr->getPrevious()->setNext(i_ptr);
      }
      if(j_ptr->getPrevious() != nullptr){
        j_ptr->getPrevious()->setNext(j_ptr);
      }
    }
  }
}